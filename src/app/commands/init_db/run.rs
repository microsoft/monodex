//! Purpose: init-db command implementation: database-state classification, deletion, creation.
//! Edit here when: Changing init-db behavior, error messages, or initialization logic.
//! Do not edit here for: Database open logic (see engine/storage/database.rs), schema definitions (see engine/schema.rs), config loading (app/config.rs).
...
pub(super) fn err_not_monodex_db(db_path: &Path) -> String {
    format!(
...
pub(super) fn err_partial_state(db_path: &Path) -> String {
    format!(
...
pub(super) fn check_existing_database_pre_lock(db_path: &Path) -> Result<Option<MetaFile>> {
...
pub(super) fn check_existing_database(db_path: &Path) -> Result<Option<MetaFile>> {
    match read_existing_database_state(db_path) {
        ExistingDatabaseState::Missing => Ok(None),
        ExistingDatabaseState::EmptyDirectory => Ok(None),
        ExistingDatabaseState::Complete(meta) => Ok(Some(meta)),
