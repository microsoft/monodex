...
The command should complete without error and create `~/.monodex/default-db/` containing `monodex-meta.json`, `chunks.lance/`, and `label_metadata.lance/`.
...
The expected output is a list of search results in `>`-prefixed format. Each result is a header line of the form:
...
### 5. Set default context (optional)
...
- The search query path is broken (vector lookup, label filtering, result formatting).
