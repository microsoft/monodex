...
- `error.rs`: Helpers for typed discrimination of Tantivy NotFound-style errors. Used by FTS read paths to normalize directory disappearance to absent-index outcomes (`open_existing` returns `Ok(None)`, `fts_search` returns `FtsSearchOutcome::NoIndex`) instead of propagating raw IO errors.
...
- `indexing.rs`: `index_chunks_for_fts` and `FtsIndexingStats`. Reads the label's chunks from LanceDB, reconciles against the staleness manifest (always cross-checking with a Tantivy term-dictionary scan), applies additions and removals, commits once, rewrites the manifest. See [crawl.md](./crawl.md) for the reconciliation rule.
- `manifest.rs`: Per-label staleness manifest at `<database-dir>/fts/<catalog>/<label>/manifest.json`. Records the `row_id`s actually live in Tantivy after commit (skipped chunks excluded), plus `FTS_SCHEMA_ID` and `FTS_TOKENIZER_ID`. Read result is the typed `ManifestRead` enum (`Missing`, `Present`, `IdMismatch`, `Unreadable`); the four cases dispatch differently.
...
- `search.rs`: `fts_search` and the `FtsHit` / `FtsSearchOutcome` result types (`Found`, `NoIndex`, `ParseError`). Builds a Tantivy query parser bound to the monodex tokenizer.
