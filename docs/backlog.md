...
## 2. Tantivy and adjacent

Full-text search is the next major investment. The work below clusters around it.

**Tantivy full-text index.** Add a Tantivy index alongside the LanceDB tables, populated during the same crawl pipeline. The index location is `<database-dir>/fts/` (see [monodex_files.md](./design/monodex_files.md)). Several open questions remain: how the index participates in the sentinel-based incremental crawl, whether existing chunk text is the right indexing input or whether it needs separate tokenization, and how the **Single-writer process lock** above coordinates writes across the two storage subsystems.

**Hybrid search via reciprocal rank fusion.** Once FTS exists, Monodex should combine vector and FTS results via RRF as the default search strategy. RRF is simple, parameter-free, and a reasonable starting point; more sophisticated fusion (learned weights, query-conditional rerankers) is a later concern. The CLI should also expose knobs for running each component in isolation, so accuracy regressions can be traced to the contributing index.

**Benchmark suite for retrieval quality.** Build a repeatable evaluation that measures retrieval quality on the Rush Stack codebase against a set of curated queries with known-good answers. The point is to make chunker changes, fusion changes, and model changes measurable rather than vibes-based.

## 3. High priority, after Tantivy

These are known-needed features whose architectural ripples are easier to reason about once Tantivy has landed and the storage layout has settled.
...
**Non-Git catalog types.** Today `monorepo` is the only catalog type. The catalog abstraction was built to accommodate non-Git sources too — issue trackers, discussion forums, meeting notes — but no concrete work is planned. This entry exists primarily to explain what "catalog" means: not a synonym for "Git repository," but a generic name for any data source Monodex indexes.
